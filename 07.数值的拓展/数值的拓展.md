...
ES6提供了二进制和八进制数值的新写法，分别是用前缀`0b`或者（`0B`）和`0o`或者`0O`。
...
从ES5开始，在严格模式下，二进制就不再允许使用前缀`0`表示，ES6 进一步明确，要使用前缀`0o`表示。
...
console.log(Number.isNaN("15")); // fasle
...
```javascript
// ES5写法
console.log(parseInt("15.6")); // 15
console.log(parseFloat("15.6")); // 15.6
...
ES6在Math对象上新增了**17个与数学相关的方法**，所以这些方法都是静态方法，只能在Math对象上调用。

### Math.trunc()去除一个数的小数部分，返回一个整数部分。

`Math.trunc()`**用于去除一个数的小数部分，返回整数部分**。

```javascript
console.log(Math.trunc(15.1)); // 15
console.log(Math.trunc(15)); // 15
console.log(Math.trunc(-15)); // 15
console.log(Math.trunc(-15.1)); // 15
```

对于非数值，首先就是在内部使用`Number`方法转为数值，这个方法内部就会直接执行这个转换的。
