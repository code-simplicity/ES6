...
ES6之前不能直接为函数的参数设置默认值，只能采用变通的方法。
...
上面代码检查函数`log()`的参数`y`有没有赋值，如果没有，则指定默认值为`world`，这种写法就的缺点就在于,如果参数`y`赋值了，但是对应的布尔值为`false`，则该值不起作用，就像上面代码最后一行，参数`y`等于空字符，结果被改为默认值。

为了避免这个问题，我们就得先判断参数`y`是否被赋值，如果没有，就等于默认值。
...
除了简洁之外，ES6的写法还有两个好处，首先就是阅读代码的人，可以立刻意识到有哪些参数是可以省略的，不用查看函数体文档，其次，有利于将来的代码优化，即使未来的版本在对外接口中彻底拿掉这个参数，也不会导致以前的代码无法运行。

参数变量是默认声明的，所以不能使用`let`或者`const`。
...
上面代码中，参数变量`x`是默认声明的，在函数体中，不能使用`let`或者`const`再次声明，否则就会报错。
...
上面代码中，参数`p`的默认值是`x + 1`，这时候，每次调用`foo()`函数都会重新计算`x + 1`，而不是默认`p`等于100。
...
ES6引入了rest参数（形式为...变量名），用于获取函数的多余参数，这样做就不再使用`arguments`对象了，rest参数搭配的变量是一个数组，该变量将多余的参数放入数组中.
...
上面代码中使用rest代替`arguments`，比较后发现，rest参数的写法更加简洁和自然。

`arguments`对象不是数组，而是一个类似数组的对象，所以为了使用数组的方法，必须使用`Array.prototype.slice.call()`先将其装换为数组，rest参数就不存在这个问题，他是一个真正的数组，数组特有的方法都可以使用，下面是利用rest参数改写数组的`push`方法的例子。
...
注意，rest参数以后不能在有其他的参数（即只有最后一个参数），否则就会报错。
...
如果**箭头函数的代码块部分多余一条语句，就要使用大括号将他括起来，并且使用`return`返回语句**
...
箭头函数使用注意点
    ...
    - **不可以当做构造函数**，也就是说不能使用`new`命令。否则就会出现异常。
    - **不可以使用`arguments`对象**，该对象在函数体内不存在，如果要用，可以使用rest参数代替。
    - **不可以使用`yield`命令**，因此箭头函数不能当做Generator函数。

切记上面几点，尤其是第一点，`this`对象的指向是可变的，都是在箭头函数中，他是固定的。
...
在上面代码中，`setTimeout`的参数是一个箭头函数，这个箭头函数的定义生效是在`foo`函数生成时，而他真正执行要等到100毫秒，如果是普通函数，`this`执行的是全局对象`window`，这时应该输出`21`，都是箭头函数导致`this`总是指向函数定义生效所在的对象里，所以输出的是`42`。

箭头函数可以让`setTimeout`里面的`this`，绑定定义时所在的作用域，而不是指向运行时所在的作用域，下面是另外一个例子。
...
上面代码中，`Timer`函数内部设置了两个定时器，分别使用了箭头函数和普通函数，前者的`this`绑定定义时所在的作用域，即（`Timer`函数），后者`this`指向运行时的作用域（全局对象），所以，3100毫秒之后，`times.s1`更新了三次，而`timer.s2`一次都没有更新。

箭头函数可以让`this`的指向固定化，这种特性很利于封装回调函数，下面的一个例子，DOM事件回调封装在一个对象值里面。
...
console.log(handle.doSomething("click")); // handleclickfor123456  undefined
```
...
由于箭头函数使得`this`从“动态”变为“静态”，下面两个场景不应该使用箭头函数。

第一个场景就是**定义对象的方法，并且该方法内部包括`this`**
