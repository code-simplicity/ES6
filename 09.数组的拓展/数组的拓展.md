...
拓展运算符（spread）是三个点（“...”），它好比restc参数的逆运算，将一个数组装化为使用逗号隔开的参数序列。
...
console.log([...document.querySelectorAll("div")]); // (3) [div, div, div]
```
...
console.log(f(-1, ...args, 2, ...[3])); // undefined
```
...
由于拓展运算符可以展开数组，所以不再需要`apply`方法，将数组装换为函数的参数。
...
下面是拓展运算符取代`apply`方法的一个实际例子，应用`Math.max`方法，简化求出一个数组最大元素的写法。

```javascript
// ES5
console.log(Math.max.apply(null, [4, 5, 1, 3])); // 5

// ES6

console.log(Math.max(...[4, 5, 1, 3])); // 5
...
另外一个例子就是使用`push`函数，将一个数组添加到另外一个数组的尾部。
...
数组是复合型的数据类型，直接复制的话，只是复制了指向底层数据结构的指针，而不是克隆一个全新的数组。
...
上面代码中，`a2`并不是`a1`的克隆，而是指向同一份数据的另外一个指针，修改了`a2`，会直接导致`a1`变化。
...
`Array.from`**将两类对象转为真正的数组**，**类似数组的对象**和**可遍历的对象**（包括ES6新增的Set和Map两种数据结构）。

下面是类似数组的对象，`Array.from`将其装换为真正的数组。
...
实际应用中，常见的类似数组的对象是DOM操作返回的NodeList的集合，以及函数内部使用的`arguments`对象，`Array.from`可以将他们转为真正的数组。
...
```javascript
console.log(Array.of(3, 10, 12)); // [3, 10, 12]
console.log(Array.of(3, 10).length); // 2
```
...
数据实例的`copyWithin()`方法，在当前数组内部，将指定位置的成员复制到其他位置（会覆盖原有的成员），然后返回当前的数组，也就是说，使用这个方法，**会修改当前数组的**。

```javascript
Array.prototype.copyWithin(target, ((start = 0), (end = this.length)));
```

- target(必须)，从该位置替换数组，如果为负值，表示倒数。
- start(可选)，从该位置读取数据，默认为0，如果为负数表示末尾开始计算。
...
数组实例的`find`方法，用于**找出第一个符合条件的数组成员**，它的参数是一个回调函数，所有的数组成员依次执行该回调函数，直接找出第一个返回值为`true`的成员。然后返回该成员，如何没有符合条件的成员，返回`undefined`。
...
```javascript
console.log(arr.findIndex((n) => n < 0)); // 3
```
...
上面代码对数组`arr`按照首字母进行排序，排序结果中，`straw`在`spork`之前，跟原始顺序一致，所以排序算法`stableSorting`是稳定排序。
